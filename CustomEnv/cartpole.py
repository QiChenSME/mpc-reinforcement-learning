...
class CartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    The default reward threshold is 500 for v1 and 200 for v0 due to the time limit on the environment.

    If `sutton_barto_reward=True`, then a reward of `0` is awarded for every non-terminating step and `-1` for
    the terminating step. As a result, the reward threshold is 0 for v0 and v1.
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        self.action_space = spaces.Discrete(2)  # 动作空间取离散空间，长度为2，从0开始（默认值）
